...
각 행을 클릭하거나 키보드 Enter/Space로 선택하면 상세 창을 엽니다. `GET /api/admin/event-logs/{id}`로 저장된 메타데이터를 다시 조회하며, 처리 결과의 의미·관련 ID 종류·오류 클래스·서버 인스턴스·생략하지 않은 발생 시각 등을 표시합니다. 본문이나 예외 원문을 추가 수집하지 않으며, 저장하지 않은 정보는 상세 창에서도 제공하지 않습니다. 보관 기간이 지났거나 없는 기록은 404로 응답합니다. 상세 창이 열린 동안 목록 자동 갱신은 멈추고, 닫기·Escape·바깥 영역 클릭으로 기존 목록에 돌아옵니다.

## 기록 범위

- Kafka: Outbox 저장 완료(트랜잭션 커밋 후), 실제 발행 성공, 재시도 예정/10회 최종 실패, 원본 알림 삭제로 인한 건너뜀.
- Kafka Consumer: 채팅 batch 저장·WebSocket 전달 처리와 알림 처리의 성공/실패. 역직렬화 등 리스너 진입 전 오류, 재시도, DLT 발행 결과도 포함합니다.
...
`referenceId`는 Kafka QUEUE/PUBLISH에서 Outbox ID, 알림 Consumer/Redis에서 알림 ID, REPLAY에서 소비 실패 기록 ID입니다. Consumer 재시도와 채팅 batch에서는 비어 있습니다. `DEAD_LETTER`는 Outbox 최종 실패, `DLT_PUBLISHED`는 소비 실패 토픽 발행 완료입니다. `DISCARDED`는 변경 전 정책의 과거 기록입니다. `DUPLICATE`는 같은 알림 이벤트의 Redis 중복 발행을 생략한 결과입니다. [실패 복구·재발행 정책](kafka-operations.md)을 참고하세요.

## 조회 / 보관

`broker=KAFKA|REDIS`, `operation=QUEUE|PUBLISH|CONSUME|CONSUMER_RETRY|REPLAY`, `outcome`으로 필터링합니다. `hours`는 1~720이며 실제 조회는 설정된 보관 기간 이내로 제한됩니다. 50개씩 기록 ID 내림차순으로 반환하고 `nextBeforeId`를 다음 요청의 `beforeId`로 넘깁니다. UI는 이전/다음, 수동 새로고침, 선택적인 10초 갱신을 지원합니다. 과거 페이지나 숨겨진 탭에서는 자동 갱신하지 않습니다. 시간은 서버 기준입니다.
